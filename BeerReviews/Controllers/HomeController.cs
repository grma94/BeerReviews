     ...
     //       ViewBag.AccessToken = _userSession.BearerToken;
        ...
        public async Task<ActionResult> SearchResults(string searchString)
        {
            if (searchString != "")
            {
                ...
                var response1 = await httpClient.GetAsync("http://52.178.159.188:8001/wa/searchbe/" + searchString);
                var beers = await response1.Content.ReadAsAsync<List<Beer>>();
                response1 = await httpClient.GetAsync("http://52.178.159.188:8001/wa/searchbr/" + searchString);
